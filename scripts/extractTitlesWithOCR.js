  ...
  try {
    ...
    try {
      ...
      if (
        ...
        logMessage('Ga naar https://cloudinary.com/console/addons om add-ons te activeren.');
      } else {
    ...
    if (!args.includes('--ignore-availability')) {
      logMessage('Script wordt gestopt. Gebruik --ignore-availability om toch door te gaan.');
      process.exit(1);
    }

    logMessage('Doorgaan ondanks mogelijke problemen met OCR beschikbaarheid...');
    return false;
  } catch (error) {
    logMessage(`Fout bij controleren van OCR beschikbaarheid: ${error.message}`);
...
async function performAlternativeOCR(publicId) {
  try {
    ...
    if (ocrInfo.info && ocrInfo.info.ocr && ocrInfo.info.ocr.adv_ocr) {
      ...
      if (ocrResult.status === 'complete' && ocrResult.data && ocrResult.data[0]) {
        ...
        // Als er textAnnotations zijn, gebruik de description van de eerste (volledige tekst)
          ...
          logMessage(`  OCR-tekst gevonden in textAnnotations: "${text}"`, true);
          return { text };
        }
      }
    }
...
async function extractTitlesWithOCR() {
  ...
  if (isDryRun) {
    console.log('DRY RUN MODUS: Er worden geen wijzigingen aangebracht in Cloudinary');
  }
  ...
  try {
    ...
    for (const painting of result) {
      ...
      try {
        ...
        if (existingCaption) {
          logMessage(`Bestaande caption gevonden voor ${painting.public_id}: "${existingCaption}"`);
          logMessage(
          ...
          if (!args.includes('--force') && !args.includes('-f')) {
            logMessage(`Overgeslagen: bestaande caption behouden voor ${painting.public_id}`);
            skipped++;
            continue;
          }
        }
        ...
        if (ocrResult && ocrResult.text) {
          ...
          if (extractedNumber) {
            logMessage(`Nummer gevonden: ${extractedNumber} in ${painting.public_id}`);

            // Update de caption in de resource
            if (!isDryRun) {
              await updateCaption(painting.public_id, extractedNumber);
              logMessage(`Caption bijgewerkt voor ${painting.public_id}: "${extractedNumber}"`);
              updated++;
            } else {
              logMessage(
            ...
            logMessage(`Geen nummer gevonden in OCR tekst voor ${painting.public_id}`);
        ...
        failed++;
        logMessage(`FOUT bij verwerken van ${painting.public_id}: ${error.message}`);
      }
...
async function getAllPaintings(folderPath) {
  ...
  try {
    ...
    console.error(`Fout bij ophalen resources uit map ${folderPath}:`, error.message);
  }
...
async function getExistingCaption(publicId) {
  try {
    ...
    console.error(`Fout bij ophalen van bestaande caption voor ${publicId}:`, error.message);
    return null;
  }
}
...
async function performOCR(publicId) {
  try {
    ...
    if (explicitResult.info && explicitResult.info.ocr && explicitResult.info.ocr.adv_ocr) {
      ...
      if (ocrResult.status === 'complete' && ocrResult.data && ocrResult.data[0]) {
        ...
        logMessage(`  OCR niet compleet of geen data gevonden. Status: ${ocrResult.status}`, true);
      }
    } else {
      ...
      if (logToFile) {
        fs.appendFileSync(
          logFilePath,
          `  Volledige OCR activatie respons: ${JSON.stringify(explicitResult, null, 2)}\n`
        );
      }
    }
    ...
    if (resourceInfo.info && resourceInfo.info.ocr && resourceInfo.info.ocr.adv_ocr) {
      ...
      if (ocrResult.status === 'complete' && ocrResult.data && ocrResult.data[0]) {
        ...
        // Als er textAnnotations zijn, gebruik de description van de eerste (volledige tekst)
          ...
          const text = ocrResult.data[0].textAnnotations[0].description;
          logMessage(`  OCR-tekst gevonden in textAnnotations bij tweede poging: "${text}"`, true);
          return { text };
        }
      }
    }
    ...
    logMessage(`  Geen OCR resultaat gevonden, alternatieve methode proberen...`);
    ...
    if (error.error) {
      logMessage(`  Error details: ${error.error.message || JSON.stringify(error.error)}`, true);
    }
...
async function updateCaption(publicId, caption) {
  try {
    ...
    console.error(`Fout bij bijwerken caption voor ${publicId}:`, error.message);
    return false;
  }
}
...
==========================================
Modus: ${isDryRun ? 'DRY RUN (geen wijzigingen)' : 'LIVE (wijzigingen toegepast)'}
...
async function main() {
  try {
    ...
    if (!args.includes('--skip-ocr-check')) {
      ...
      logMessage('OCR beschikbaarheidscontrole overgeslagen door --skip-ocr-check optie');
    }
